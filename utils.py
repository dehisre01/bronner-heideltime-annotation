...
month_dict = {"Jan":"01", "Januar":"01", "Jänner":"01", "Feb":"02", "Februar":"02", "Hörnung":"02", "März":"03", "April": "04",
"May":"05", "Jun": "06", "Juni": "06", "Jul":"07", "Julius":"07", "Aug":"08", "August":"08", "Sept":"09", "September":"09",
"Oct":"10", "October":"10", "Nov":"11", "November":"11", "Dec":"12", "December":"12"}

# regex pattern to match months (Bronners early 19th century German)
month_pattern = '(Jan(uar)?|Jänner|Feb(ruar)?|Hörnung|Mär(z)?|Apr(il)?|May|Jun(i)?|Jul(ius)?|Aug(ust)?|Sept(ember)?|Oct(ober)?|Nov(ember)?|Dec(ember)?)'
...
# this usually happens if heideltime identifies a shorter component of an allready annotated date
        ...
        if date.find_previous_sibling() == None: # prevents None has not attribute name errors
            ...
            if len(date['when']) == len(date.find_previous_sibling()['when']): # probably there are two dates following each other,
                # if they are 12 days appart they are gregorian and julian for the same date and will be annotated regarding 
            ...
            elif str(date.previous_sibling) == " ": # two following date tags annotate components of the same date and will be merged
        ...
        if type(date.previous_sibling) == bs4.element.NavigableString:
            ...
            if re.search('\d{1,2}(\.|(te|ten))', str(date.previous_sibling)) and date.string:
                match = re.search('\d{1,2}', str(date.previous_sibling))
                start = match.start()
            ...
            if re.search('\d{1,2}\D+\d{1,2}\D+', str(date.string)): # brauchsch die str()?
...
def refine_time(bs_soup_object):
    """This function takes an bs.soup object of a TEI annotated XML and refines the automated annotations generated by 'heideltime' """
        ...
        if "when" in time.attrs:
            ...
            if re.search('(halb|[123]/[24])', time.previous_sibling):
                ...
                if re.search('(halb|1/2)', time.previous_sibling):
                    ...
                    if new_time < 10:
                        time['when'] = "0" + str(new_time) + ":30"
                    else:
                        time['when'] = str(new_time) + ":30"
                    if time.string == None: # if there is a selfclosing tag (<lb/>) included in the tag, the string method is not working (unfortunally I could find no fix)
                        pass
                    else:
                ...
                if re.search('3/4', time.previous_sibling):
                    new_time = int(time['when'][:2]) - 1
                    if new_time < 10:
                        time['when'] = "0" + str(new_time) + ":45"
                    else:
                        time['when'] = str(new_time) + ":45"
                    if time.string == None: # if there is a selfclosing tag (<lb/>) included in the tag, the string method is not working (unfortunally I could find no fix)
                        pass
                    else:
                        time.string.replace_with(time.previous_sibling[start:]+time.string)
