...
# heideltime_parser.set_document_type('NEWS') # default = narratives narratives is able to update the document time dynamically
heideltime_parser.set_language('GERMAN') # default = english
heideltime_parser.set_document_time('1810-01-01') # default = today

ns = "http://www.tei-c.org/ns/1.0"

tree =  etree.parse("data/Bronner_test.xml")
...
for i in tqdm(time_elements, desc="writing time annotations"):
    ...
    if view.get_table_pos(int(i.attrib['begin'])) in so.table.df.position.values:
    ...
    # references, like "PRESENT_REF" in the text e.g. "jetzt" have no value for us
        ...
        if i.attrib['timexType'] == "DATE":
            if int(i.attrib['timexValue'][:4]) < 1800: 
                ...
                pass
            elif ("-SU" or "-FA" or "-WI" or "-SP") in i.attrib['timexValue']:  # das klappt nicht, aber warum!!!?????
                ...
                'calendar':'#julian'} 
                # julian is set as default calendar
                )
        else:
            if "P" in i.attrib['timexValue']: # a value starting with P is a clear indication for a duration
                if "X" in i.attrib['timexValue']: # a value including PX is an undefined (or by heideltime not defineable) time,
                ...
                if re.match(r"\d{4}(-\d{2})+", i.attrib['timexValue']):
                    ...
                    tag='time',
                    depth=3,
# instead of a pattern like "1810-02-15T05:00" only the time "05:00" is written as value of the "when" attribute
                    attrib={'when':i.attrib['timexValue'][-5:]}
                    )
                else:
                    so.add_inline(
                    begin=view.get_table_pos(int(i.attrib['begin'])),
...
so.tree.write('data/output.xml', encoding='utf-8')


with open('data/output.xml', 'r', encoding='utf-8') as file:
    doc = file.read()
